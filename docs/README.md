...
- `DecimalString` — a decimal number string matching `-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+-]?[0-9]+)?` — the JSON number grammar, e.g. `"1.5"`, `"-0.25"`, `"6.02e23"` — or one of the exact-case specials `"NaN"`, `"Infinity"`, `"-Infinity"`. Signed zero is valid (floats distinguish it); `"+1.5"`, `".5"` and `"inf"` are not. String storage makes float round-trips deterministic across serialisers.
