...
import { renderEnumeration } from '../src/renderEnumeration';
import { defineEnumeration, variant } from './_fixtures';

describe('renderEnumeration', () => {
    it('emits a string-literal union with sorted variants', () => {
        const e = defineEnumeration('Endianness', {
            variants: [variant('le'), variant('be')],
        });
        const result = renderEnumeration(e);
    ...
    it('produces a fragment with no imports', () => {
        ...
        expect(renderEnumeration(e).imports.size).toBe(0);
    });
